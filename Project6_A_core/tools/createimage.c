...
static task_info_t0 tasks[TASK_MAXNUM];
...
static void create_image(int nfiles, char *files[])
{
    ...
    for (int fidx = 0; fidx < nfiles; ++fidx)
    {
        ...
        if (taskidx >= -1)
        {
            taskinfo[taskidx + 1].start_address = phyaddr;
            memcpy(tasks[taskidx + 1].name, *files, 16);
        }
        /* open input file */
        fp = fopen(*files, "r");
        assert(fp != NULL);
        ...
        for (int ph = 0; ph < ehdr.e_phnum; ph++)
        {
            ...
            {
                tasks[taskidx + 1].vaddr += phdr.p_vaddr;
                printf("tasks vaddr %x\n", tasks[taskidx].vaddr);
            }
        }
        printf("memsz is %x\n", memsz);
        taskinfo[taskidx + 1].task_memsz = memsz;
        /* write padding bytes */
        /**
         * TODO:
        ...
        if (taskidx >= -1)
        {
            taskinfo[taskidx + 1].end_address = phyaddr - 1;
        }
        printf("addr is : %x\n", phyaddr);
        fclose(fp);
        files++;
                           ...
                           short tasknum, FILE *img)
{
    ...
    {
        short start_block = taskinfo[i].start_address / SECTOR_SIZE;
        short num_block = taskinfo[i].end_address / SECTOR_SIZE - start_block + 1;
        int offset = taskinfo[i].start_address % SECTOR_SIZE;
        tasks[i].start_block = start_block;
        tasks[i].num_block = num_block;
        tasks[i].offset = offset;
    }
    short loc_block = tasks[tasknum].start_block + tasks[tasknum].num_block;
    fseek(img, 0x1f8, SEEK_SET);
    fwrite(&loc_block, 2, 1, img);
    printf("loc block is %d\n", loc_block * SECTOR_SIZE);
    fseek(img, 0x1fa, SEEK_SET);
    fwrite(&tasknum, 2, 1, img);

    short sector_num = nbytes_kern / SECTOR_SIZE + (nbytes_kern % SECTOR_SIZE != 0);
    printf("sector is %d\n", sector_num);
    fseek(img, OS_SIZE_LOC, SEEK_SET);
    fwrite(&sector_num, 2, 1, img);
    fseek(img, loc_block * SECTOR_SIZE, SEEK_SET);
    for (int i = 0; i <= tasknum; i++)
    {
        printf("tasks name %s\n", tasks[i].name);
        printf("tasks vaddr %x\n", tasks[i].vaddr);
    }
    fwrite(&tasks, sizeof(task_info_t0) * (tasknum + 1), 1, img);
}
