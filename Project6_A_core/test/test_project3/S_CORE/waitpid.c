...
int main(int argc, int arg0)
{
    ...
    //assert(rte_id != -1);
    char *s1[]= {"1","20","22"};
    pid_t pid1 =  sys_exec("ready_to_exit", 3, s1/*print_location + 1, handle1, handle2*/);
    ...
    //assert(wl_id != -1);
    char *s2[3]= {"2","20","22"};
    pid_t pid2 = sys_exec("ready_to_exit", 3, s2/*print_location + 2, handle1, handle2*/);
